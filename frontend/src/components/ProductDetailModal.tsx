...
import {
  ...
  ProductionCenterItem, PriceZonesMap, MenuLevelItem
} from '../types';
...
type TabType = 'general' | 'prices' | 'pos' | 'rules' | 'menu';
...
export const ProductDetailModal: React.FC<ProductDetailModalProps> = ({
  ...
  const [menuLevels, setMenuLevels] = useState<MenuLevelItem[]>([]);
  ...
  useEffect(() => {
    if (product && isOpen) {
      ...
      if (product.menu_levels && product.menu_levels.length > 0) {
        setMenuLevels(product.menu_levels);
      } else if (product.is_menu) {
        setIsLoadingMenu(true);
        fetch(`/api/products/${product.codigo}`)
          .then(res => res.ok ? res.json() : null)
          .then(data => {
            if (data && data.menu_levels) {
              setMenuLevels(data.menu_levels);
            }
          })
          .catch(err => console.error("Erro ao carregar estrutura do menu:", err))
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-1 text-xs text-slate-400">
                ...
                {product.is_menu && (
                  <span className="inline-flex items-center gap-1 text-purple-200 bg-purple-950/80 border border-purple-700 px-2.5 py-0.5 rounded-full text-[10px] font-bold shadow-xs">
                    <Utensils className="w-2.5 h-2.5 text-purple-300" />
                    Menu / Combo (ZSRest)
                  </span>
                )}
              </div>
            ...
            <button
              ...
              className={`py-3 px-3 text-xs font-bold border-b-2 flex items-center gap-1.5 transition ${
                activeTab === 'menu'
                  ...
                  : 'border-transparent text-purple-700 hover:text-purple-900 hover:bg-purple-100/60'
              }`}
            >
              ...
              {menuLevels.length > 0 && (
                <span className="ml-1 px-1.5 py-0.2 bg-purple-100 text-purple-800 rounded-full text-[10px] font-bold">
                  {menuLevels.length} {menuLevels.length === 1 ? 'nível' : 'níveis'}
                </span>
              )}
            </button>
          )}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          {activeTab === 'general' && (
            <div className="space-y-4">
              {/* Menu Banner */}
              {product.is_menu && (
                <div className="bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-200 rounded-xl p-3.5 flex items-center justify-between shadow-xs">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        <h4 className="text-xs font-black text-purple-950 uppercase tracking-wider">Artigo Tipo Menu / Composto (ZSRest)</h4>
                        <span className="px-2 py-0.2 text-[10px] font-bold bg-purple-200 text-purple-900 rounded-md">composto = 2</span>
                      </div>
                      <p className="text-[11px] text-purple-800 mt-0.5">
                        {menuLevels.length > 0
                          ? `Este menu possui ${menuLevels.length} níveis de seleção e artigos suplementares configurados.`
                          : 'Menu configurado no ZoneSoft com suporte a níveis e opções.'}
                      </p>
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={() => setActiveTab('menu')}
                    className="text-xs font-bold bg-purple-600 hover:bg-purple-700 text-white px-3.5 py-1.5 rounded-lg transition shadow-xs flex items-center gap-1.5 shrink-0"
                  >
          ...
          {activeTab === 'menu' && (
            <div className="space-y-4">
              <div className="bg-purple-50/70 border border-purple-200 rounded-xl p-4 flex items-center justify-between">
                <div>
                  <h3 className="text-sm font-bold text-purple-950 flex items-center gap-2">
                    ...
                    Estrutura de Níveis e Opções do Menu
                  </h3>
                  <p className="text-xs text-purple-800 mt-0.5">
                    Configuração em tempo real extraída diretamente das tabelas de menu do ZoneSoft (<code className="font-mono text-purple-900 bg-purple-100 px-1 py-0.5 rounded">dbo.niveismenu</code> e <code className="font-mono text-purple-900 bg-purple-100 px-1 py-0.5 rounded">dbo.niveismenuext</code>).
                  </p>
                </div>
                {isLoadingMenu && (
                  <span className="text-xs text-purple-700 font-semibold animate-pulse flex items-center gap-1 shrink-0">
              ...
              {isLoadingMenu && menuLevels.length === 0 ? (
                <div className="p-8 text-center text-slate-500 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                  ...
                  <p className="text-xs font-semibold">A carregar níveis e opções do menu...</p>
                </div>
              ) : menuLevels.length === 0 ? (
                <div className="p-8 text-center text-slate-500 bg-slate-50 rounded-xl border border-dashed border-slate-300 space-y-2">
                  <Utensils className="w-8 h-8 mx-auto text-slate-400" />
                  <p className="text-xs font-bold text-slate-700">Nenhum nível configurado para este menu</p>
                  <p className="text-[11px] text-slate-500 max-w-md mx-auto">
                    Este artigo está identificado como Menu (<code className="font-mono">composto = 2</code>), mas não possui linhas associadas em <code className="font-mono">dbo.niveismenu</code>.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {menuLevels.map((lvl) => (
                    <div
                      key={lvl.nivel}
                      className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-xs hover:border-purple-300 transition"
                    >
                      {/* Level Header */}
                      <div className="bg-slate-50/90 px-4 py-2.5 border-b border-slate-200 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="w-6 h-6 rounded-full bg-purple-100 text-purple-800 font-extrabold text-xs flex items-center justify-center border border-purple-200 shadow-2xs">
                            {lvl.nivel}
                          </span>
                        ...
                        <span className="text-[11px] text-slate-500 font-medium">
                          {lvl.options.length} {lvl.options.length === 1 ? 'artigo disponível' : 'artigos disponíveis'}
                        </span>
                      </div>
                      ...
                      {lvl.options.length === 0 ? (
                        <div className="p-4 text-center text-xs text-slate-400 italic">
                          Sem artigos associados a este nível.
                        </div>
                      ) : (
                        <div className="divide-y divide-slate-100">
                          {lvl.options.map((opt) => (
                            <div
                              key={opt.codigo}
                              className="px-4 py-2 text-xs flex items-center justify-between hover:bg-slate-50/60 transition"
                            >
