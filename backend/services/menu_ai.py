...
def parse_plain_text_menu(text: str) -> MenuExtractionResponse:
    """Faz parsing heurístico de texto de ementa (secções, artigos e preços)."""
    ...
    for line in lines:
        # Se for tudo maiúsculas e sem preço -> nova secção
...
def export_zs_import_template_csv(rows: List[MenuReviewedRow]) -> str:
    """Gera CSV compatível com o importador de artigos da ZoneSoft."""
    output = io.StringIO()
    writer = csv.writer(output, delimiter=';')
    # Cabeçalho ZoneSoft oficial
    writer.writerow([
...
def convert_matched_to_import_rows(rows: List[MenuReviewedRow], price_mapping: Dict[str, str]) -> List[ImportRow]:
    """Converte linhas da ementa associadas a artigos existentes em ImportRow para apply_import."""
    import_rows: List[ImportRow] = []
    for r in rows:
        if not r.matched_codigo:
            continue
        item = ImportRow(codigo=r.matched_codigo)
        if r.descricaocurta:
            item.descricaocurta = r.descricaocurta
        if r.selected_familia:
            item.familia = r.selected_familia
